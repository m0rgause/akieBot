...
const its = (query) => new Promise((resolve, reject) => {
    ...
    fetchJson(`http://api.kikiaf.art/v1/igstory?username=${query}&apikey=${config.vhtear}`)
...
const ytSearch = (query) => new Promise((resolve, reject) => {
    ...
    fetchJson(`http://api.kikiaf.art/v1/ytsearch?query=${encodeURI(query)}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const ytPlay = (query) => new Promise((resolve, reject) => {
    ...
    fetchJson(`http://api.kikiaf.art/v1/ytsearchdl?query=${encodeURI(query)}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
